...
API_ID = int(environ.get('API_ID', '22343331'))
API_HASH = environ.get('API_HASH', '18c5b0aa2494e24b642ec640b94e7399')
BOT_TOKEN = environ.get('BOT_TOKEN', "7108392620:AAE0Vsu1ueSnFWnZ4KcAF4GNDoxcXCHlmq8")
...
STREAM_SITE = (environ.get('STREAM_SITE', 'https://clicksfly.com'))
STREAM_API = (environ.get('STREAM_API', 'a2249dd7847494dcf251c6cb32025f60bba9392a'))
STREAMHTO = (environ.get('STREAMHTO', 'https://t.me/Ultroid_Official/18'))

# Admins, Channels & Users
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '5152954203').split()]
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002122756793 -1002126327884').split()] #Channel id for auto indexing ( make sure bot is admin )
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '5152954203').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
PREMIUM_USER = [int(user) if id_pattern.search(user) else user for user in environ.get('PREMIUM_USER', '').split()]
auth_channel = environ.get('AUTH_CHANNEL', '-1002129185203') #Channel / Group Id for force sub ( make sure bot is admin )
auth_grp = environ.get('AUTH_GROUP')
AUTH_CHANNEL = int(auth_channel) if auth_channel and id_pattern.search(auth_channel) else None
AUTH_GROUPS = [int(ch) for ch in auth_grp.split()] if auth_grp else None
support_chat_id = environ.get('SUPPORT_CHAT_ID', '-1002116588946') # support group id ( make sure bot is admin )
reqst_channel = environ.get('REQST_CHANNEL_ID', '-1002059781481') # request channel id ( make sure bot is admin )
...
DATABASE_URI = environ.get('DATABASE_URI', "mongodb+srv://thor99:thor999999@cluster0.kwanebd.mongodb.net/?retryWrites=true&w=majority")
...
MSG_ALRT = environ.get('MSG_ALRT', 'ᴍᴀɪɴᴛᴀɪɴᴇᴅ ʙʏ : Ghost')
LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1002063112575')) #Log channel id ( make sure bot is admin )
SUPPORT_CHAT = environ.get('SUPPORT_CHAT', 'https://t.me/Ghost_Bot_py') #Support group link ( make sure bot is admin )
...
FILE_STORE_CHANNEL = [int(ch) for ch in (environ.get('FILE_STORE_CHANNEL', '-1002137159156')).split()]
